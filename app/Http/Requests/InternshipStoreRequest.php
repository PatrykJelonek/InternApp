...
class InternshipStoreRequest extends FormRequest
{
    ...
    public function authorize()
    {
        ...
        if (Auth::user()->hasRole(RoleConstants::ROLE_STUDENT)) {
            $haveActiveInternship =  (bool) Auth::user()->student->internships()->whereHas('status', function (Builder $query) {
                $query->where(['name' => [InternshipStatusConstants::STATUS_NEW, InternshipStatusConstants::STATUS_ACCEPTED, InternshipStatusConstants::IN_PROGRESS]]);
            })->count();

            return !$haveActiveInternship;
        }
